...
Resposta:
Porque a função deve ser responsável apenas por processar os dados, e não por decidir como eles serão apresentados ao usuário. Ao retornar desconto, totalFinal e error, ela pode ser reutilizada em diferentes contextos (terminal, API, testes, interface gráfica etc.). A responsabilidade de exibir mensagens fica na main, seguindo o princípio da separação de responsabilidades. Isso torna o código mais organizado, reutilizável e fácil de testar.